...
#             print >>sys.stderr, '%s = %s' % (name, value)
    ...
    def start_test(self, test):
        if self._show_all:
            self._stream.write(self.colorize("testname", "Testing "))
            self._stream.writeln(self.colorize("testname",
    ...
    def test_error(self, test, exc_info, label):
        if self._show_all:
            self._stream.write(self.colorize("error",
                                             self.get_description(test)))
            self._stream.write(self.colorize("error", " ... "))
    ...
    def test_failure(self, test, exc_info):
        if self._show_all:
            self._stream.write(self.colorize("failure",
                                             self.get_description(test)))
            self._stream.write(self.colorize("failure", " ... "))
